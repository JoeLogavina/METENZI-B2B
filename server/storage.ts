...
export class DatabaseStorage implements IStorage {
  ...
  async getCompanyHierarchy(parentCompanyId: string): Promise<{ mainCompany: User; branches: User[] } | null> {
    // Get the main company
    const [mainCompany] = await db
      .select()
      .from(users)
      .where(eq(users.id, parentCompanyId));
    ...
    // Get all branches
    const branches = await this.getUserBranches(parentCompanyId);
  ...
  async createBranchUser(parentCompanyId: string, branchData: {
    ...
    // Get parent company to inherit some properties
    ...
    // Create branch user with proper hierarchy
    const [newBranch] = await db
      ...
      .values({
        ...
        firstName: branchData.branchName.split(' ')[0] || 'Branch',
        lastName: branchData.branchName.split(' ').slice(1).join(' ') || 'User',
        role: 'b2b_user',
        ...
        // Inherit some properties from parent company
        country: parentCompany.country,
        city: parentCompany.city,
        vatOrRegistrationNo: parentCompany.vatOrRegistrationNo,
      })
      .returning();
